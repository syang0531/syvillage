...
## POI 점유는 demote 때 반납한다

바닐라 주민은 **죽을 때만** 침대·작업대·집회장 점유를 반납한다(`Villager.releaseAllPois`, `die()` 안에 있다). 우리 엔티티는 죽지 않고 **끊임없이 떠난다** — demote마다, 청크 언로드마다, 민병대 교체마다. 그리고 `discard()`는 아무것도 반납하지 않는다.

그래서 왕복마다 **침대 하나가 존재하지 않는 엔티티에게 점유된 채 남는다.**

게임 테스트 하루 만에 마을의 침대 3개가 전부 유령 소유가 되었고, 주민 6명 전원이 잘 곳이 없다고 보고했다. 한 방향으로만 진행되는 누수다. 방치하면 **플레이어가 두 번 방문한 마을은 영구히 침대가 없어지고**, M2의 수용력이 침대에서 나오므로 **아이가 영영 태어나지 않는다.**

demote·교체 시 `HOME`·`JOB_SITE`·`MEETING_POINT` 점유를 직접 반납한다. **기억은 지우지 않는다** — promote 때 복원된 브레인이 그 침대를 기억하고 있고, 점유가 실제로 비어 있으므로 다시 가져갈 수 있다.

> 일반화하면 이렇다. **바닐라가 "죽을 때" 하는 정리는 우리에게 "내려갈 때"다.** 엔티티가 사라지는 경로가 바닐라보다 훨씬 많다는 것이 이 모드의 구조적 특징이고, 바닐라가 그 자리에 놓아둔 정리 코드는 전부 한 번씩 점검해야 한다.
