...
| # | 기준 | 상태 |
|---|---|---|
| 1 | 밤에 주민이 전원 실내에 있다 | ✅ 통금 개입 확인. 문 닫기는 M3에서 구현(`DoorWatch`) |
...
| # | 기준 | 상태 |
|---|---|---|
| 1 | 인구가 늘면 집이 자동으로 지어지고 수용력이 올라간다 | ❌ **집 미구현. 고리가 아직 안 닫힘** |
| 2 | L2 건설이 promote 시 재생된다 | ✅ 성벽 1621 op를 가상으로 완성 후 재생 확인 |
| 3 | 경사지에서 안 깨지고 5블록 이상 단차는 생략 | 🔶 링 587칸 중 538칸 배치(49칸 생략). 눈으로 재확인 필요 |
| 4 | 주민이 성문을 통과해 길찾기한다 | 🔶 문 7개 실제 생성 확인. 통행은 미관측 |
| 5 | 짓다 만 건물을 부숴도 무한 루프가 없다 | ❌ 미구현 (`verifySampleEvery`, `maxRebuildAttempts`) |

### 성벽 파이프라인에서 게임 테스트가 찾은 것

| 증상 | 원인 |
|---|---|
| 자재 충분한데 `QUEUED`에서 안 넘어감 | 단계 머신은 L2에서만 돌고 작업은 L0에서만 함. **장부와 노동을 한 모듈에 묶었다** |
| 벽이 24/1618에서 멈춤 | op 정렬이 `(y, x, z)`. 둘레에서 그건 순서가 아니라 **뒤섞기** — 연속한 두 블록이 160칸 떨어진다 |
| 97/1618에서 또 멈춤 | 배치를 주민 위치에 걸었다. 링 폭 168칸은 어떤 반경으로도 못 덮는다 |
| 떠나면 5배 느려짐 | **서로 맞아야 하는 config 키 두 개.** 맞을 리가 없다 |
| info는 문 7개, 땅에는 0개 | 가상 완료 작업을 큐에서 즉시 제거. **재생은 그 큐를 순회한다** |
| 문이 통나무 밑에 묻힘 | 게이트 칸을 op에서 **뺐다**. 빼는 것은 치우는 것이 아니다 |
| 나무꾼이 계속 사라짐 | write-back이 바닐라 직업으로 덮어씀. 나무꾼은 바닐라 눈에 nitwit |

일곱 중 여섯이 **LOD 경계**에서 나왔다. L0과 L2가 같은 상태를 다른 규칙으로 다루면, 어느 한쪽만 보고 있는 동안에는 전부 정상으로 보인다.
