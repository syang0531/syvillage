...
차단은 Mixin이 아니라 이벤트로 한다. `BabyEntitySpawnEvent`에서 부모가 등록된 마을 소속이면 취소한다. 26.2에서도 이 이벤트가 `ICancellableEvent`를 구현하므로 취소가 가능하다는 것을 확인했다. 등록되지 않은 마을은 이 판정에서 즉시 빠져나가므로 비용이 0이다.
