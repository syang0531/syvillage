...
바닐라 주민은 나이로 죽지 않는다. 노화사는 **순전히 이 모드가 더한 개념**이다.

이 모드의 명제는 *"주민이 왜 죽었는지 알 수 있게 한다"*이지 *"더 많이 죽게 한다"*가 아니다. 노화는 그 명제에 아무것도 보태지 않으면서 테스트 마을을 반복해서 죽였다 — 편입된 주민이 전부 같은 나이로 시작하던 시절엔 마을 전체가 같은 보름에 죽었고, 나이를 흩뿌린 뒤에도 수용력에 붙은 마을은 여전히 말라갔다.

기능은 남긴다. 세대가 이어지는 정주지를 원하는 플레이어에게는 그게 재미의 일부다. `enableAging = true`.
