...
작업대도 같다. 여러 개 놓아도 되고(발전도는 이미 최고 수위라 아무 일도 안 생긴다), 부숴도 발전도는 내려가지 않는다.

### 해제는 명령어로만

서바이벌에서 해제할 방법은 없고, 그게 의도다. 재등록하면 **발전도 래칫이 `timber`로 리셋**되고 마을 이름도 바뀐다 — 블록 상호작용으로 열어두기에는 위험이 비대칭이다. 그리고 실수로 등록될 일도 거의 없다: 그냥 우클릭은 종을 울리고, 등록은 **시프트**+우클릭이다.
