...
1b. **타입을 바꿀 때는 `Codec.withAlternative`로 옛 모양을 함께 읽는다.** 저장된 필드는 모양을 *추가*할 수는 있어도 *교체*할 수는 없다. M1에서 `scale`을 `ScaleTier`(문자열)에서 `ScaleState`(맵)로 바꿨다가 기존 세이브가 `Not a map: "outpost"`로 거부됐다 — 2번 방어가 아니었으면 마을 하나가 조용히 비워졌을 것이다
