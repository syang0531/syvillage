...
`PopulationTest`. 확률로 굴러가는 것은 **플레이로 확인할 수 없다.** 0.0067/step을 100스텝 돌려서 0회가 나올 확률이 51%다. 죽은 코드와 정상 동작을 눈으로 구별할 수 없다는 뜻이고, 그래서 충분히 길게 돌려서 답을 받는다.

| 테스트 | 확인하는 것 | 완료 기준 |
|---|---|---|
| `growsToCapacityAndHolds` | 출산이 실제로 일어나고, 수용력에서 멈춘다 | 1 |
| `bedsBindGrowth` | 침대 0이면 한 명도 안 태어나고, 병목이 BEDS로 지목된다 | 2 |
| `warningPrecedesFamine` | 경고가 첫 아사보다 **먼저** 기록된다 | 3 |
| `ageingIsNotBirth` | 생애 단계 변화가 BIRTH로 기록되지 않는다 | — |

### 픽스처를 나눈 이유

`SettlementFixture.full`은 왕복 테스트용이다. 모든 필드가 기본값이 아니어야 누수를 잡을 수 있으므로 플롯(침대 3)과 채워진 사상자 링을 들고 있고, 그 둘이 **수용력을 인구 아래로 눌러둔다.** 그 위에서 성장 테스트를 돌리면 통과하지만 아무것도 증명하지 못한다 — 이미 꽉 찬 마을은 아이가 안 생기는 것이 정답이기 때문이다.

`SettlementFixture.adopted`는 M2가 실제로 만나는 모양이다. 플롯 없음(건설은 M3), 침대는 앵커에서, 사상자 없음, 기근 플래그 없음.

### 이 테스트들을 쓰다 걸린 것들

- `catchUp`은 `maxCatchupTicks`를 넘으면 **요약한다.** 한 번에 2000스텝을 요청하면 360스텝만 돌고 나머지는 건너뛴다. 길게 돌릴 때는 쪼개서 호출할 것.
- 연대기는 200개를 넘으면 오래된 것부터 버린다. 긴 기근을 돌리면 찾으려던 경고가 밀려나 있다.
- 픽스처의 `foodWarned = true`가 경고를 정상적으로 억제한다. 기근 테스트에는 갓 편입된 마을이 필요하다.
