...
### 시계는 LOD 전환 경계에서만 움직인다

정산과 실체화의 순서가 틀리면 **마을이 스스로 굶는다.**

시뮬레이션 모듈은 `MATERIALIZED` 주민을 건너뛴다(이중 계산 방지). 그런데 마을 단위 집계인 **소비는 전원을 센다.** 따라서 주민에게 몸이 있는 상태로 `catchUp`이 돌면 생산 0 · 소비 전액이 되어 재고가 순수하게 깎인다.

규칙은 둘이다.

| 전환 | 해야 할 일 |
|---|---|
| 아무도 없음 → 누군가 실체화 | **먼저 `catchUp`을 끝내고** 나서 `MATERIALIZED`로 바꾼다 |
| 마지막 주민이 내려감 | 실체화 상태로 보낸 시간을 **시뮬레이션하지 않고 시계만 넘긴다** |

두 번째는 그동안 벌어진 일이 이미 월드에서 실제로 벌어졌고 writeBack으로 회수되었기 때문이다.

**promote 경로만 이걸 지키면 부족하다.** 플레이어가 돌아오면 청크에 저장돼 있던 주민 엔티티가 먼저 올라와 `EntityJoinLevelEvent`에서 **스스로 재바인딩**한다. 그러면 `materializedCount`가 0이 아니게 되어 promote 작업이 아예 시작되지 않고, 자리 비운 기간이 정산되지 않은 채 남는다. 나중에 아무 명령어나 치는 순간 그 기간이 "아무도 일하지 않은 시간"으로 정산된다.

게임 테스트에서 실제로 나왔다. 농부 4명이 장부에 있는데 생산 모듈은 0명을 봤고, 3분 자리를 비운 대가로 밀 96이 사라졌다.

```
step 69: 4 farmer(s) of 6 resident(s) produced 12 wheat
step 70: 0 farmer(s) of 6 resident(s) produced 0 wheat   ← 재바인딩 이후
```

재바인딩·청크 언로드·엔티티 퇴장 **모든 경로**가 이 규칙을 지켜야 한다.
...
## 앵커 — M1·M2가 M3를 기다리지 않게 하는 장치

`docs/defense.md`의 실내 판정·감시 지점·무기고, `docs/population.md`의 침대 수는 전부 **플롯에서 온다.** 그런데 플롯은 M3에서야 생긴다. 로드맵 순서가 M1 → M2 → M3이므로, 그대로 두면 앞의 두 마일스톤이 만들어지지 않은 것을 읽는다.

해법은 출처를 감추는 것이다.

```
AnchorSet          ← M1·M2가 읽는 유일한 창구
├─ shelters        실내로 삼을 지점
├─ watchPoints     감시 지점
├─ muster          집결지
└─ bedCount        수용력
...
| | M1~M2 | M3 이후 |
|---|---|---|
| shelters | 침대 POI | `BUILT` 셀 |
| watchPoints | 종 + 최외곽 거주 지점 | 감시탑 + `GateNode` |
| muster | **종** | `ARMORY` 플롯 |
| bedCount | 침대 POI 수 | 플롯 침대 |

등록된 바닐라 마을은 이미 침대와 종을 갖고 있다. 백 게임일을 버틴 마을이 자기 사람들이 어디서 자는지 모를 리 없는데, M3를 기다리느라 그걸 무시하면 앞의 두 마일스톤을 **검증조차 할 수 없다.**

집결지가 종인 것은 타협이 아니다. 무기고 경유는 원래 **엔티티 교체를 눈에 안 띄게 가리는 연출**이 목적이고, 종을 울려 모이는 것은 플레이어가 이미 기대하는 그림이다.

### 앵커는 조회하지 않고 캐시한다

POI 조회는 청크를 요구하고, 그건 원칙 2가 금지한다. 그래서 **실체화된 동안 스캔해 `Settlement`에 저장**하고, 가상 쪽은 저장본을 읽는다. `anchorRefreshTicks`(기본 6000 = 5분)마다 갱신한다.

조금 오래된 대피소 목록은, 청크를 로드해야만 시뮬레이션되는 마을보다 훨씬 작은 문제다.
