...
public final class WallPlan {
    ...
    public static final Identifier RAMPART =
            Identifier.fromNamespaceAndPath(SyVillage.MODID, "wall/rampart");
    ...
    public static Optional<BuildRecipe> plan(ServerLevel level, Settlement settlement,
        ...
        int batch = SyVillageConfig.WALL_COLUMNS_PER_JOB.get();
        ...
        SyVillage.LOGGER.debug("'{}' has {} column(s) of wall to raise", settlement.name(),
                todo.size());
    ...
    private static int heightOf(ServerLevel level, Settlement settlement, BlockPos bell,
        ...
        if (lowest == Integer.MAX_VALUE
                || highest - lowest > SyVillageConfig.MAX_CELL_SLOPE.get()) {
            return Ground.SKIP;
        }
        return highest;
    }
