...
public final class RoadPlan {
    ...
    public static final Identifier STREET =
            Identifier.fromNamespaceAndPath(SyVillage.MODID, "road/street");
    ...
    public static Optional<BuildRecipe> plan(ServerLevel level, Settlement settlement,
        ...
        int batch = SyVillageConfig.ROAD_BLOCKS_PER_JOB.get();
        ...
        for (int ring = 0; ring <= limit && todo.size() < batch; ring++) {
            for (int along = -ring; along <= ring && todo.size() < batch; along++) {
                for (BlockPos pos : new BlockPos[] {
                    ...
                    if (TownPlan.beyondTheLastRing(pos, settlement)
                            && !TownPlan.inArch(pos, bell)) {
                        continue;   // side streets end at the ring road; the bell's go through
                    }
                    if (!reach.street(pos)) {
        ...
        SyVillage.LOGGER.debug("'{}' has {} block(s) of street to lay in phase {}",
