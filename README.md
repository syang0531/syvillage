...
현재 상태: **M1 구현 완료.** 마을을 등록하면 주민이 이름을 갖고, 플레이어가 없는 동안 데이터로만 존재하며 식량을 생산하고 소비한다. 방어·인구 성장·건설은 M1~M3.
...
- [x] **M0** 데이터 레이어 — record + Codec, SavedData, 등록, `catchUp`, promote/demote
- [x] **M1** 안전 — 야간 대피, 경보 상태, 민병대, L2 습격 공식 *(계수 보정은 진행 중)*
- [ ] **M2** 인구 — 수용력, 출산, 기아, 연대기
- [ ] **M3** 건설 — 파이프라인, 성벽 → 집 → 밭
